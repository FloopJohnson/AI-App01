...
// ==========================================
// FASTENER SKU GENERATION
// ==========================================

const FASTENER_CATEGORY_PREFIXES = {
    'Bolts': 'BOLT',
    'Nuts': 'NUT',
    'Washers': 'WASH',
    'Screws': 'SCREW',
...
/**
 * Generate category prefix for fasteners
 */
export const getFastenerCategoryPrefix = (categoryName) => {
    if (FASTENER_CATEGORY_PREFIXES[categoryName]) {
        return FASTENER_CATEGORY_PREFIXES[categoryName];
    }

    // For custom categories, use first 4 letters uppercase
    return categoryName.replace(/\s+/g, '').substring(0, 4).toUpperCase();
};

/**
 * Get the next available SKU for a fastener category
 * Format: PREFIX-XXX (e.g., BOLT-001, NUT-042)
 */
export const generateNextFastenerSKU = async (categoryName) => {
    try {
        const prefix = getFastenerCategoryPrefix(categoryName);

        // Query all fasteners with SKUs starting with this prefix
        const fastenersRef = collection(db, 'fastener_catalog');
        const snapshot = await getDocs(fastenersRef);
        ...
        console.error('[SKU] Error generating fastener SKU:', error);
        throw new Error('Failed to generate fastener SKU');
    }
};
...
export const checkFastenerSKUExists = async (sku, excludeFastenerId = null) => {
    try {
        const fastenersRef = collection(db, 'fastener_catalog');
        const q = query(fastenersRef, where('sku', '==', sku));
        const snapshot = await getDocs(q);

        // If editing a fastener, exclude it from the check
        if (excludeFastenerId) {
            return snapshot.docs.some(doc => doc.id !== excludeFastenerId);
        }

        return !snapshot.empty;
    } catch (error) {
        console.error('[SKU] Error checking fastener SKU:', error);
        return false;
    }
};
